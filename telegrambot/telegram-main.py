...
def ask_level(update, context):  
    level_text = \
"""Heyyo! I am RC4's Laundry Bot. <i>As I am currently in [BETA] mode, I can only show details for Ursa floor.</i>\

<b>Which laundry level do you wish to check?</b>"""

    level_buttons = []
    for level in LAUNDRY_LEVELS:
        buttons = InlineKeyboardButton(text=f"Level {level}", callback_data=f"check_L{level}")
...
Last updated: {current_time}\n"""
    ...
    layout = [[InlineKeyboardButton("Refresh now", callback_data="refresh"),
...
def set_reminder(update, context):
    ...
    layout = [ 
            [InlineKeyboardButton(k + ('✓' if v else '') , callback_data=k) for k,v in memory.items()],
    ]
...
def main() -> None:
    ...
    top_conv = ConversationHandler(
        ...
        states={
            MAIN_MENU: [CommandHandler("view_status", ask_level),
                        CallbackQueryHandler(level_status,
            ...
            SET_REMINDER: [CallbackQueryHandler(set_reminder),
